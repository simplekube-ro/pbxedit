...
- `pbxedit --version` reports the release version.
...
- CI gains a required oracle lane: every post-operation fixture is read by `xcodebuild -list`.
...
- New: `.github/workflows/release.yml`, an `oracle` job in `ci.yml`, `docs/RELEASING.md`, `LICENSE`, `README.md` sections, a version constant.
...
- Can start once `integrity-rules-lint` has produced an executable; should ship after `lint-fix` so 1.0 contains all v1 commands.
