...
The targets and `platformFilters` for `<to>` SHALL be determined as `add` determines them — `--target` and `--platform`, then configuration, then sibling inference at the destination — for the file's kind, taken from its extension or else from the phase its build files are in. Where they differ from the file's current membership, the command SHALL detach the file from targets no longer chosen (as `remove --target` does), attach it to newly chosen targets (as `add` does), set or remove `platformFilters` on retained build files, and list each such change in the output. When nothing differs, nothing SHALL be emitted for membership. With `--keep-membership`, membership and filters SHALL be left exactly as they were; the destination's siblings then decide nothing, and a note SHALL say when they belong to other targets.
...
- **WHEN** `App/iOS/Panel.swift` with `platformFilters = (ios, )` moves to `App/Shared/Panel.swift`, whose sibling carries no filter
- **THEN** the build file `BB0000000000000000000190` keeps its ID, loses `platformFilters`, and the output lists the change
...
- **THEN** the command exits `1` listing the variants and asking for `--target` or `--keep-membership`, and writes nothing
