...
| `settings-conflict` | `settings = {COMPILER_FLAGS = "-w"; };` on `BB0000000000000000000140`, the build file of `App/Filtered/F1.swift` in `App` | that file re-filtered (`remove --target App`, `add --target App --platform ios,macos`) and `settings = {COMPILER_FLAGS = "-Wall"; };` on the build file it then holds | exit `3`: the unit offers `ours` or `theirs-membership` over the conflicting `settings`, reported with both sides' values (issue #28); with `theirs-membership`, exit `0`, theirs' filters and the `settings` owed |
...
| `reordered-array` | `knownRegions = (Base, en, );` — base's two regions swapped | `knownRegions = (en, Base, fr, );` — one inserted | exit `3`: the hunk over `knownRegions` offers `ours` and `theirs` only, though its own texts look like two insertions (issue #24); `both` for it is exit `2`. The hunk covers the whole array (issue #32): with `theirs`, exit `0` and `(en, Base, fr, )`; with `ours`, exit `0` and `(Base, en, )` |
