...
enum Fixtures {
    static let directory: URL = URL(fileURLWithPath: #filePath)
        .deletingLastPathComponent()  // PBXModelTests
...
func plutilLint(_ bytes: [UInt8]) throws -> (ok: Bool, output: String) {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("pbxedit-\(UUID().uuidString).pbxproj")
...
/// Serializes, asserts the output re-parses and `plutil -lint` accepts it, and returns the text.
func serialized(_ project: Project, file: StaticString = #filePath, line: UInt = #line) -> String {
    let output = project.serialize()
    switch SyntaxTree.parse(output) {
    case .success(let tree):
        XCTAssertTrue(tree.serialize() == output, "the output does not round-trip", file: file, line: line)
    case .failure(let error):
        XCTFail("the output does not parse: \(error)", file: file, line: line)
    }
    assertPlutilLints(output, file: file, line: line)
    return text(output)
}
