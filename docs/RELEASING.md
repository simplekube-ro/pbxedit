...
| `v1.5.0` | — | `9b8db7e` | **not run** — skipped by the release owner's decision (2026-09-24); the oracle lane (`xcodebuild -list` on every post-operation fixture, `move` among them) passed on the merged commit, so Xcode can read the result, but nothing was measured about Xcode leaving the written bytes alone |
