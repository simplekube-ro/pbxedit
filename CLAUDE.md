...
`pbxedit` is a Swift CLI that manages **file membership** in an Xcode `project.pbxproj` (`add`, `move`, `remove`, `lint [--fix]`, `query`, and a semantic three-way `merge`). It was built through a chain of ten OpenSpec changes against the approved design, and has grown by further changes since. Shipped so far: layer 1, `PBXSyntax` (`lossless-syntax-tree`), layer 2, `PBXModel` (`typed-project-model`), the rule set plus `lint` and the CLI skeleton (`integrity-rules-lint`: `Sources/PBXOps/Rules/`, `Sources/pbxedit/`), `query` (`query-command`: `Sources/PBXOps/Query/`), `add` with the Plan/conventions/runner machinery (`add-command`: `Sources/PBXOps/{Plan,Inference,Add}/`), `.pbxedit.yml` with the config layer of `Conventions`, lint exemptions and the baseline default (`conventions-config`: `Sources/PBXOps/Config/`, Yams), `remove` with the shared operation renderer (`remove-command`: `Sources/PBXOps/Remove/`, `Sources/pbxedit/{Remove,OperationReport}.swift`), `move` with directory moves and `--keep-membership` (`move-command`: `Sources/PBXOps/Move/`), `lint --fix` with whole-project verification (`lint-fix`: `Sources/PBXOps/Repair/`, `Sources/pbxedit/RepairReport.swift`), and the Xcode-canonical platform-filter spelling (`platform-filter-canonical-form`: `Sources/PBXOps/Inference/PlatformFilters.swift`, Xcode-saved evidence under `Tests/Fixtures/xcode27/`), all archived under `openspec/changes/archive/`; `openspec list --specs` shows what has landed. `release-distribution` is archived too: `v1.0.0` is released (2026-09-22) from `release.yml`, installed with `brew install simplekube-ro/tap/pbxedit`, and every release follows `docs/RELEASING.md`, including the manual Xcode open-and-save check. `merge-command` (issue #9, for `v1.1.0`) adds `pbxedit merge <base> <ours> <theirs>`: membership replayed through the planners, the rest merged as text, checks A–F before any write, exit `3` with a decisions template (`Sources/PBXOps/Merge/`, `Sources/pbxedit/{Merge,MergeReport}.swift`, three-way fixtures under `Tests/Fixtures/merge/` regenerated with `PBXEDIT_WRITE_MERGE_FIXTURES=1 swift test --filter MergeFixtureFileTests`); it is archived, merged (PR #10) and released as `v1.1.0` (2026-09-23). `merge-accounting-shared-leaves` (issue #12) fixes check C for a leaf that several decided hunks govern (merged, PR #14); `merge-both-unordered-insertions` (issue #13) offers `both` for different insertions into one unordered array (`Sources/PBXOps/Merge/UnorderedInsertions.swift`). Both are archived, merged and released as `v1.1.1` (2026-09-23). `merge-both-multiline-objects` (issue #17, for `v1.1.2`) offers `both` for a multi-line object both sides insert at one place — a Swift package, a group — by trying the untrimmed union when zealous trimming has cut the trimmed `ours + theirs` apart, and builds a hunk's base counterfactual from its whole stretch so a base that never held the trimmed frame is not handed it twice (`Sources/PBXOps/Merge/{LineMerge,Hunks}.swift`, fixture `Tests/Fixtures/merge/both-objects/`); it is archived, merged (PR #18) and released as `v1.1.2` (2026-09-23). New work starts a new change.
...
4. **CLI** (`Sources/pbxedit/`) — `--dry-run` prints plan and unified diff; `--json` everywhere. Write is temp-file-then-rename, then the written bytes are re-parsed and re-checked. Exit codes: `0` success or no-op, `1` rule violation or refused operation, `2` usage or parse error, `3` decisions needed (`merge` only).
...
Decided 2026-09-22 (`TODO.md` § 10, `docs/design.md` § Open items): the name stays `pbxedit` despite `ZehMatt/PBXEdit`; the licence is MIT (`LICENSE`); the Homebrew tap is `simplekube-ro/homebrew-tap`, which `release.yml` updates through the `TAP_TOKEN` organization secret. Nothing blocks publishing; every release follows `docs/RELEASING.md`.
